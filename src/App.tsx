...
/* ─────────────────────────────────────────────────────────────────────────
 * ASSET MAP — Single source of truth for all section media.
 * day = light-mode asset · night = dark-mode asset
 * ───────────────────────────────────────────────────────────────────────── */
 ...
 * Renders a <video> or <img> depending on the file extension.
 * Active layer gets opacity:1 / zIndex:2; inactive gets opacity:0 / zIndex:1.
 * EVERY <video> always has autoPlay loop muted playsInline in camelCase.
 * ───────────────────────────────────────────────────────────────────────── */
...
function SectionMedia({ src, isActive }: SectionMediaProps) {
  ...
  return <img src={src} alt="" style={style} aria-hidden="true" />;
}
...
const REVOLUTION_CARDS = [
  ...
  },
  {
    ...
    details: [
      ...
      "Henry Ford introduziu a linha de montagem in 1913, reduzindo o tempo de produção do Ford T de 12h para 93 minutos.",
  ...
  },
  {
    key: "4a-revolucao",
    icon: "brain" as const,
    tags: ["2010–PRESENTE", "CONECTIVIDADE"],
    title: "4ª Revolução",
    subtitle: "Inteligência & Conectividade",
    desc: "Sistemas ciberfísicos, IoT, nuvem e inteligência artificial fundem os mundos físico e digital de forma hiperconectada.",
    details: [
      "A inteligência artificial e machine learning otimizam processos produtivos autônomos e preditivos em tempo real.",
      "A Internet das Coisas (IoT) conecta bilhões de dispositivos para coleta e tomada de decisões baseada em dados.",
      "Gêmeos Digitais e simulação em nuvem minimizam falhas ao testar virtualmente antes da implantação física.",
      "O SENAI lidera o desenvolvimento dessa força de trabalho técnica apta a gerenciar ecossistemas 4.0 complexos.",
    ],
  },
];
...
export default function App() {
   ...
   *  Pull-down transform is applied via inline style (NOT a CSS class)
   *  to avoid !important conflicts with opacity on the bg videos. */
  ...
  /**
   * Pull-down physics applied as inline styles (avoids CSS !important conflict
   * with opacity on inactive background videos).
   */
  const pullTransform: React.CSSProperties = isPullingDown
    ? {
      transform: "translateY(20vh) scale(1.1)",
      transition: "transform 0.3s var(--pull-easing), opacity 0.3s ease",
    }
    : {
      transform: "none",
  ...
  /* ── RENDER ───────────────────────────────────────────────────────────── */
  return (
    <div className={`w-full min-h-screen ${isDayMode
      ...
      {/* ══════════════════════════════════════════════════════════════════
          GLOBAL FIXED HERO BACKGROUND
          Both videos always present; active layer at zIndex:2, opacity:1.
          inactive at zIndex:1, opacity:0. Pull-down via inline style only
          (no CSS class) so !important never overrides opacity.
      ══════════════════════════════════════════════════════════════════ */}
      <div
        style={{
          position: "fixed",
          ...
          pointerEvents: "none",
          zIndex: 0,
          isolation: "isolate",   /* stacking context for child z-index */
        }}
      >
        {/* ── Dark video: background-dark.mp4 — active in night mode ── */}
        <video
          autoPlay
          loop
          muted
          playsInline
          src={SECTION_MEDIA.hero.night}
          style={{
            position: "absolute",
            inset: 0,
            width: "100%",
            ...
            zIndex: isDayMode ? 1 : 2,
            ...pullTransform,
          }}
        />

        {/* ── Light video: background-light.mp4 — active in day mode ── */}
        <video
          autoPlay
          loop
          muted
          playsInline
          src={SECTION_MEDIA.hero.day}
          style={{
            position: "absolute",
            inset: 0,
            width: "100%",
            ...
            zIndex: isDayMode ? 2 : 1,
            ...pullTransform,
          }}
        />

        {/* Subtle scrim above both videos */}
        <div
          style={{
            position: "absolute",
            inset: 0,
      ...
      {/* ══════════════════════════════════════════════════════════════════
          NAVIGATION — fixed top-left (logo + toggle) & top-right (links)
      ══════════════════════════════════════════════════════════════════ */}

      {/* ── Logo — fixed top-left ─────────────────── */}
      <div className="fixed top-5 left-5 md:top-8 md:left-8 z-[100] flex items-center">
        {/* Logo → Home */}
        <button
          onClick={() => handleScrollTo("hero", "Início")}
          className={`w-10 h-10 flex items-center justify-center rounded-full overflow-hidden cursor-pointer border-0 hover:scale-105 transition-all p-1 ${themeGlass}`}
      ...
      <div className="fixed top-5 right-[72px] md:top-8 md:right-8 z-[100] flex items-center">
        <button
          onClick={handleThemeToggle}
          aria-label={`Modo ${isDayMode ? "escuro" : "claro"}`}
          aria-pressed={isDayMode}
          className={`relative w-[112px] h-10 rounded-full p-1 flex items-center cursor-pointer border-0 overflow-hidden outline-none transition-all duration-500 shadow-md ${themeGlass}`}
        >
          <div className="absolute inset-0 flex items-center justify-between px-3.5 text-[10px] font-bold tracking-wider uppercase pointer-events-none select-none font-body">
          ...
          <div className={`absolute top-[3px] bottom-[3px] w-8 h-8 rounded-full flex items-center justify-center shadow-[0_4px_12px_rgba(0,0,0,0.18)] border transition-all duration-500 pointer-events-none ${isDayMode ? "left-[3px] bg-white border-slate-200" : "left-[calc(100%-35px)] bg-white/20 border-white/30"}`}>
            {isDayMode
      ...
      {/* ══════════════════════════════════════════════════════════════════
          HORIZONTAL PILL NAV BAR — fixed, centered at top of viewport
          Inspired by image-1 reference: active-pill | flat links | CTA
      ══════════════════════════════════════════════════════════════════ */}
      <nav
        aria-label="Navegação principal"
        style={{ position: "fixed", top: "1.25rem", left: "50%", transform: "translateX(-50%)", zIndex: 100 }}
      >
        <div className={`h-nav-bar${isDayMode ? " h-nav-bar--day" : ""}`}>

          {/* ── Active section label — left pill ── */}
          <span
            className={`h-nav-active${isDayMode ? " h-nav-active--day" : ""}`}
            aria-current="page"
          >
            {NAV_ITEMS.find((i) => i.id === activeSection)?.label ?? "SENAI"}
          </span>

          {/* ── Vertical divider ── */}
          <div className="h-nav-divider" aria-hidden="true" />

          {/* ── Flat section links ── */}
          {([
            { id: "revolucoes",   label: "4 Eras"   },
            { id: "industria-4",  label: "Ind. 4.0"  },
            { id: "legado-senai", label: "Legado"    },
          ] as const).map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
              onClick={(e) => { e.preventDefault(); handleScrollTo(item.id, item.label); }}
              aria-current={activeSection === item.id ? "page" : undefined}
              className={`h-nav-link${
                activeSection === item.id ? " h-nav-link--active" : ""
              }${isDayMode ? " h-nav-link--day" : ""}`}
            >
              {item.label}
            </a>
          ))}

          {/* ── Portal SENAI CTA — blue pill ── */}
          <a
            href="https://sorocaba.sp.senai.br"
            target="_blank"
            rel="noopener noreferrer"
            className="h-nav-portal"
            aria-label="Portal SENAI (nova aba)"
          >
            Portal SENAI ↗
          </a>
        </div>
      ...
      <button
        ...
        className={`fixed top-5 right-5 z-[100] md:hidden w-10 h-10 flex items-center justify-center rounded-full cursor-pointer transition-colors border-0 outline-none ${themeGlass} ${themeText}`}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className={`fixed inset-x-0 top-[72px] mx-4 z-40 rounded-2xl md:hidden flex flex-col p-5 space-y-3 shadow-2xl ${themeGlass} ${isDayMode ? "text-stone-900" : "text-white"}`}
          >
      ...
      {/* ══════════════════════════════════════════════════════════════════
          SEÇÃO HERO
          Background: global fixed videos (background-light.mp4 / background-dark.mp4)
      ══════════════════════════════════════════════════════════════════ */}
      <section
        id="hero"
        aria-label="Início — As Quatro Revoluções Industriais e o SENAI"
      ...
      {/* ══════════════════════════════════════════════════════════════════
          SEÇÃO: REVOLUÇÕES OVERVIEW
          Background: revs.gif — both light AND dark modes
          Timeline added below the three revolution cards
      ══════════════════════════════════════════════════════════════════ */}
      <section
        id="revolucoes"
        aria-label="As Quatro Eras da Revolução Industrial"
      ...
      >
        ...
        <div className="section-bg">
          ...
          <div
            ...
            style={{
              background: isDayMode ? "rgba(220,215,208,0.58)" : "rgba(0,0,0,0.65)",
              zIndex: 2,
            }}
            aria-hidden="true"
          />
        ...
        {/* Content */}
        <div className="section-content flex flex-col px-8 md:px-16 lg:px-20 pt-28 pb-24 max-w-7xl mx-auto w-full">

          <div className="text-left w-full">
            <span className={`text-[11px] sm:text-xs font-mono tracking-[0.25em] uppercase block mb-4 ${isDayMode ? "text-stone-600" : "text-white/60"}`}>
              02 // AS QUATRO ERAS DA INDÚSTRIA
            </span>
            <h2
              className={`font-heading italic text-5xl md:text-7xl lg:text-[6.2rem] leading-[0.85] tracking-[-3px] max-w-5xl text-left ${isDayMode ? "text-stone-950" : "text-white"}`}
              style={headingShadow}
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-16 w-full">
            {REVOLUTION_CARDS.map((card) => (
              <div
                ...
                className={`rounded-[1.25rem] p-6 min-h-[360px] flex flex-col justify-between cursor-pointer group hover:scale-[1.02] active:scale-[0.99] transition-all duration-300 border ${
                  isDayMode
                    ? "bg-white/30 border-black/[0.07] backdrop-blur-sm"
                    : "liquid-glass border-white/5 bg-white/0"
                }`}
              >
                <div className="flex items-start justify-between gap-4">
                  <div className={`w-11 h-11 rounded-[0.75rem] flex items-center justify-center border group-hover:scale-105 transition-all ${
                    isDayMode ? "bg-black/[0.04] border-black/10" : "liquid-glass bg-white/[0.02] border-white/10"
                  }`}>
                    {card.icon === "cog"     && <Cog     className={`w-5 h-5 ${isDayMode ? "text-stone-800" : "text-white"}`} aria-hidden="true" />}
                    {card.icon === "zap"     && <Zap     className={`w-5 h-5 ${isDayMode ? "text-stone-800" : "text-white"}`} aria-hidden="true" />}
                    {card.icon === "monitor" && <Monitor className={`w-5 h-5 ${isDayMode ? "text-stone-800" : "text-white"}`} aria-hidden="true" />}
                    {card.icon === "brain"   && <Brain   className={`w-5 h-5 ${isDayMode ? "text-stone-800" : "text-white"}`} aria-hidden="true" />}
                  </div>
                  <div className="flex flex-wrap justify-end gap-1.5 max-w-[70%] select-none">
                    {card.tags.map((tag) => (
                      <span key={tag} className="stat-pill">{tag}</span>
                    ))}
                  </div>
                </div>
                <div className="mt-8 text-left">
                  <h3 className={`font-heading italic text-3xl md:text-4xl tracking-[-1px] leading-none mb-1 ${isDayMode ? "text-stone-950" : "text-white"}`}>{card.title}</h3>
                  <p className={`text-xs font-body uppercase tracking-widest mb-3 ${isDayMode ? "text-stone-500" : "text-white/50"}`}>{card.subtitle}</p>
                  <p className={`text-sm font-body font-light leading-snug max-w-[32ch] ${isDayMode ? "text-stone-700" : "text-white/80"}`}>{card.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ══════════════════════════════════════════════════════════════════
          ...
          Day:   1revday.png   (image → <img>)
          Night: 1revnight.mp4 (video → <video autoPlay loop muted playsInline>)
          Layout: 2-col — giant typographic year + narrative content
      ══════════════════════════════════════════════════════════════════ */}
      <section
        id="revolucao-1"
        aria-label="A Primeira Revolução Industrial — A Era do Vapor"
        className={`section-container ${isDayMode ? "theme-light" : "theme-dark"}`}
      >
        <div className="blur-overlay blur-overlay-top" aria-hidden="true" />
        <div className="blur-overlay blur-overlay-bottom" aria-hidden="true" />
        ...
        <div className="section-bg">
          {/* Night: video — active in dark mode */}
          <SectionMedia src={SECTION_MEDIA.rev1.night} isActive={!isDayMode} />
          {/* Day:   image — active in light mode */}
          <SectionMedia src={SECTION_MEDIA.rev1.day} isActive={isDayMode} />
          <div className="section-overlay" style={{ background: overlay(), zIndex: 3 }} aria-hidden="true" />
        </div>
        ...
        <div className="section-content px-8 md:px-16 lg:px-20 py-28 max-w-7xl mx-auto w-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center min-h-[70svh]">
            ...
            <div className="flex flex-col justify-center">
              ...
              <h2
                className={`font-heading italic text-5xl md:text-6xl lg:text-7xl leading-[0.85] tracking-[-3px] mb-6 anim-child ${isDayMode ? "text-stone-950" : "text-white"}`}
                style={headingShadow}
              >
                A Era do Vapor e da Mecanização
              </h2>
              <p
                className={`text-base md:text-lg font-body font-light leading-relaxed max-w-lg mb-8 anim-child ${isDayMode ? "text-stone-800" : "text-white/80"}`}
                style={bodyShadow}
              >
              ...
              <button
                onClick={() => handleScrollTo("revolucao-2", "2ª Revolução")}
      ...
      {/* ══════════════════════════════════════════════════════════════════
          ...
          Day:   2rev.png      (image → <img>)
          Night: 2revnight.mp4 (video → <video autoPlay loop muted playsInline>)
          Layout: Centered cinematic headline + floating stat pills
      ══════════════════════════════════════════════════════════════════ */}
      <section
        id="revolucao-2"
        aria-label="A Segunda Revolução Industrial — Eletricidade e Escala"
        className={`section-container ${isDayMode ? "theme-light" : "theme-dark"}`}
      >
        <div className="blur-overlay blur-overlay-top" aria-hidden="true" />
        <div className="blur-overlay blur-overlay-bottom" aria-hidden="true" />
        ...
        <div className="section-bg">
          <SectionMedia src={SECTION_MEDIA.rev2.night} isActive={!isDayMode} />
          <SectionMedia src={SECTION_MEDIA.rev2.day} isActive={isDayMode} />
          <div className="section-overlay" style={{ background: overlay("rgba(0,0,0,0.5)", "rgba(238,235,225,0.25)"), zIndex: 3 }} aria-hidden="true" />
        </div>
        ...
        <div className="section-content flex flex-col items-center justify-center px-8 md:px-16 py-32 text-center max-w-5xl mx-auto w-full min-h-[100svh]">
          ...
          <h2
            className={`font-heading italic text-5xl md:text-7xl lg:text-[6rem] leading-[0.85] tracking-[-3px] mb-8 anim-child ${isDayMode ? "text-stone-950" : "text-white"}`}
            style={headingShadow}
          >
            Eletricidade, Escala e a Linha de Montagem
          </h2>
          <p
            className={`text-base md:text-xl font-body font-light leading-relaxed max-w-2xl mb-10 anim-child ${isDayMode ? "text-stone-800" : "text-white/80"}`}
            style={bodyShadow}
          >
      ...
      {/* ══════════════════════════════════════════════════════════════════
          ...
          Day:   3revday.mp4   (video → <video autoPlay loop muted playsInline>)
          Night: 3revnight.mp4 (video → <video autoPlay loop muted playsInline>)
          Layout: Full-bleed, content bottom-anchored (cinema style)
      ══════════════════════════════════════════════════════════════════ */}
      <section
        id="revolucao-3"
        aria-label="A Terceira Revolução Industrial — O Alvorecer Digital"
        className={`section-container ${isDayMode ? "theme-light" : "theme-dark"}`}
      >
        <div className="blur-overlay blur-overlay-top" aria-hidden="true" />
        <div className="blur-overlay blur-overlay-bottom" aria-hidden="true" />
        ...
        <div className="section-bg">
          <SectionMedia src={SECTION_MEDIA.rev3.night} isActive={!isDayMode} />
          <SectionMedia src={SECTION_MEDIA.rev3.day} isActive={isDayMode} />
          <div className="section-overlay" style={{ background: overlay("rgba(0,0,0,0.52)", "rgba(235,238,245,0.28)"), zIndex: 3 }} aria-hidden="true" />
        </div>
        ...
        <div className="section-content flex flex-col justify-end px-8 md:px-16 lg:px-20 pb-24 max-w-7xl mx-auto w-full min-h-[100svh]">
          <h2
            className={`font-heading italic text-5xl md:text-7xl lg:text-[6.5rem] leading-[0.85] tracking-[-4px] mb-6 anim-child ${isDayMode ? "text-stone-950" : "text-white"}`}
            style={headingShadow}
          >
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-end gap-8">
            <div className="flex-1">
              ...
              <p
                className={`text-base md:text-lg font-body font-light leading-relaxed max-w-xl anim-child ${isDayMode ? "text-stone-800" : "text-white/80"}`}
                style={bodyShadow}
              >
      ...
      {/* ══════════════════════════════════════════════════════════════════
          ...
          Day:   4rev.mp4      (video → <video autoPlay loop muted playsInline>)
          Night: 4revnight.mp4 (video → <video autoPlay loop muted playsInline>)
          Layout: 2-col — text left, feature grid right
      ══════════════════════════════════════════════════════════════════ */}
      <section
        id="industria-4"
        aria-label="A Indústria 4.0 — Inteligência Artificial e Ecossistema Hiperconectado"
        className={`section-container ${isDayMode ? "theme-light" : "theme-dark"}`}
      >
        <div className="blur-overlay blur-overlay-top" aria-hidden="true" />
        <div className="blur-overlay blur-overlay-bottom" aria-hidden="true" />
        ...
        <div className="section-bg">
          <SectionMedia src={SECTION_MEDIA.rev4.night} isActive={!isDayMode} />
          <SectionMedia src={SECTION_MEDIA.rev4.day} isActive={isDayMode} />
          <div className="section-overlay" style={{ background: overlay("rgba(0,0,0,0.55)", "rgba(238,240,245,0.3)"), zIndex: 3 }} aria-hidden="true" />
        </div>
        ...
        <div className="section-content px-8 md:px-16 lg:px-20 py-28 max-w-7xl mx-auto w-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center min-h-[70svh]">
            ...
            <div className="flex flex-col justify-center">
              ...
              <button
                onClick={() => handleScrollTo("legado-senai", "Legado SENAI")}
                className={`anim-child self-start text-sm tracking-widest uppercase font-bold transition-all border-b border-transparent pb-0.5 hover:border-current cursor-pointer select-none ${isDayMode ? "text-stone-900" : "text-white"}`}
              >
                O Papel do SENAI →
              </button>
      ...
      {/* ══════════════════════════════════════════════════════════════════
          ...
          Background: Pure black #000000 — cinematic conclusion
          Layout: Centered wordmark + historical timeline + CTA
      ══════════════════════════════════════════════════════════════════ */}
      <section
        id="legado-senai"
        aria-label="O Legado do SENAI Sorocaba"
        className="section-container theme-dark"
      ...
      >
        ...
        <div className="section-content flex flex-col items-center justify-center px-8 md:px-16 py-32 text-center max-w-5xl mx-auto w-full min-h-[100svh]">
          ...
          <div className="mb-2 overflow-hidden" aria-hidden="true">
            <span
              className="font-heading italic select-none pointer-events-none block"
          ...
          <h2
            className="font-heading italic text-white text-4xl md:text-6xl lg:text-7xl leading-[0.9] tracking-[-3px] mb-8 max-w-3xl anim-child"
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center anim-child">
            ...
            <button
              onClick={() => { setIsModalOpen(true); triggerNotification("Abrindo formulário de visita..."); }}
          ...
          <div className="mt-24 flex flex-col items-center gap-3">
            ...
            <p className="text-[10px] font-mono text-white/20 tracking-[0.3em] uppercase">
              SENAI Sorocaba · Indústria 4.0 · {new Date().getFullYear()}
            </p>
          </div>
        </div>
      </section>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              role="dialog"
              aria-modal="true"
              aria-label="Agendamento de visita ao SENAI"
              className="w-full max-w-md rounded-2xl bg-[#0d0d0d] border border-white/10 p-6 sm:p-8 relative z-10 shadow-2xl"
            >
              <div className="flex justify-between items-center pb-4 border-b border-white/10 mb-6">
                <div className="flex items-center gap-2">
                  <CalendarDays className="w-4 h-4 text-indigo-400" aria-hidden="true" />
                  <span className="text-[10px] uppercase tracking-[0.2em] font-bold text-white/80">Agendar Visita</span>
                </div>
                <button
                  onClick={() => setIsModalOpen(false)}
                  aria-label="Fechar modal"
                  className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-white/10 text-white/60 hover:text-white transition-all cursor-pointer border-none outline-none"
                >
              ...
              {!registered ? (
                <form onSubmit={handleRegister} className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-semibold tracking-wider text-stone-300 uppercase" htmlFor="select-course">
                      Curso de Interesse
                    </label>
                    <select
                      id="select-course"
                      ...
                      className="w-full bg-white/5 border border-white/10 focus:border-white rounded px-4 py-3 text-sm text-white focus:outline-none transition-all font-sans cursor-pointer"
                    >
                      {["Mecatrônica", "Eletrotécnica", "Automação Industrial", "IA Aplicada", "Manufatura Avançada", "Robótica Industrial"].map((c) => (
        ...
        {infoPopupContent && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              role="dialog"
              aria-modal="true"
              aria-label="Marcos históricos da revolução"
              className={`w-full max-w-lg rounded-2xl p-6 sm:p-8 relative z-10 shadow-2xl border transition-all duration-300 ${isDayMode ? "bg-stone-50 border-stone-300 text-stone-900" : "liquid-glass text-white border-white/10"}`}
            >
